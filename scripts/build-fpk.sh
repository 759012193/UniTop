#!/usr/bin/env bash
# =============================================================================
# 构建 UniTop 飞牛 FnOS 安装包 (.fpk)
#
# 直接使用官方 fnpack CLI 打包（fnpack 在生成 .fpk 前会校验 manifest / cmd /
# config / wizard / app 等结构，确保产物与飞牛 fnOS 安装校验完全一致）。
#
# 用法（在仓库根目录执行):
#   bash scripts/build-fpk.sh docker      # 构建 Docker 版  -> dist/UniTop-fnos-docker-<ver>.fpk
#   bash scripts/build-fpk.sh native      # 构建本地版(非Docker) -> dist/UniTop-fnos-native-<ver>.fpk
#   bash scripts/build-fpk.sh             # 两个都构建
#
# 环境变量：
#   FPK_NAME_PREFIX  输出文件名前缀，默认 "unitop"
#                    例如 FPK_NAME_PREFIX=UniTop-fnos 会生成 UniTop-fnos-docker-<ver>.fpk / UniTop-fnos-native-<ver>.fpk
#   FPK_ITER         迭代号，默认空
...
echo "[build-fpk] UniTop 版本: $VER"

# 输出文件名前缀与迭代号（由 CI 传入，实现 UniTop-fnos-docker-0.9.30.fpk 风格）
PREFIX="${FPK_NAME_PREFIX:-unitop}"
ITER_SUFFIX=""
if [ -n "${FPK_ITER:-}" ]; then
