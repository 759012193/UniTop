#!/usr/bin/env python3
"""Stamp the UniTop version into Wails desktop metadata copies."""
...
_MANIFEST_IDENTITY = re.compile(
    r'(<assemblyIdentity\b[^>]*\bname="com\.unitop\.desktop"[^>]*\bversion=")[^"]+(")',
    re.IGNORECASE,
)
...
def stamp_manifest(src: Path, dest: Path, version: str) -> None:
    ...
    if n != 1:
        raise SystemExit(f"unitop assemblyIdentity not found in {dest}")
